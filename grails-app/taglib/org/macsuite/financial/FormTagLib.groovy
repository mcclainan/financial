...
class FormTagLib implements ApplicationContextAware, InitializingBean {
    ...
    Closure datePicker = { attrs ->
        ...
        if (precision >= PRECISION_RANKINGS["day"]) {
            out.println "<select class=\"form-control date-picker\" name=\"${name}_day\" id=\"${id}_day\""
            if (attrs.disabled) {
        ...
        if (precision >= PRECISION_RANKINGS["month"]) {
            out.println "<select class=\"form-control date-picker\" name=\"${name}_month\" id=\"${id}_month\""
            if (attrs.disabled) {
                out << ' disabled="disabled"'
            }
            if (attrs.readonly) {
        ...
        if (precision >= PRECISION_RANKINGS["year"]) {
            out.println "<select class=\"form-control date-picker\" name=\"${name}_year\" id=\"${id}_year\""
            if (attrs.disabled) {
                out << ' disabled="disabled"'
            }
            if (attrs.readonly) {
        ...
        if (precision >= PRECISION_RANKINGS["hour"]) {
            out.println "<select class=\"form-control date-picker\" name=\"${name}_hour\" id=\"${id}_hour\""
            if (attrs.disabled) {
                out << ' disabled="disabled"'
            }
            if (attrs.readonly) {
        ...
        // do minute select
        if (precision >= PRECISION_RANKINGS["minute"]) {
            out.println "<select class=\"form-control date-picker\" name=\"${name}_minute\" id=\"${id}_minute\""
            if (attrs.disabled) {
                out << 'disabled="disabled"'
            }
            if (attrs.readonly) {
